...
export const listTiersSchema = z.object({
  ...
  active: z.string().transform((val) => (val === "true" ? 1 : val === "false" ? 0 : undefined)).optional(),
...
export const getCustomerTiersSchema = z.object({
  customerId: z.string().regex(/^\d+$/, "Invalid customer ID").transform(Number),
});
...
export type AssignTierToCustomerInput = z.infer<typeof assignTierToCustomerSchema>;
export type RemoveTierFromCustomerInput = z.infer<typeof removeTierFromCustomerSchema>;
